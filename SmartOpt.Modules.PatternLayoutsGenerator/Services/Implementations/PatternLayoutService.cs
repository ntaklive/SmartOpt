...
public class PatternLayoutService : IPatternLayoutService
{
        ...
        double maxWaste, int groupSize)
    {
        ...
        var elementsForSplittingExist = true;
        ...
        while (elementsForGroupingExist)
        {
            ...
            {
                if (elementsForSplittingExist && TryFindSuitableOrderIndexForSplitting(orders, out int suitableOrderForSplittingIndex))
                {
            ...
            while (groupWaste > maxWaste || groupWaste < 0)
            {
                ...
                else
                {
                    if (elementsForSplittingExist && TryFindSuitableOrderIndexForSplitting(orders, out int suitableOrderForSplittingIndex))
                    {
                        ...
                        patternLayoutReady = false;
                        break;
                    }

                    elementsForSplittingExist = false;
            ...
            if (!patternLayoutReady)
            {
                continue;
            }

            patternLayouts.Add(new PatternLayout(ordersGroup, groupWaste));
    ...
    {
        return (1 - (double) ordersGroup.Sum(x => x.Width) / maxWidth) * 100;
    }
